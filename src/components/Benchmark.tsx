...
const GenericCell = memo(({ children, isUpdated }: { children: React.ReactNode; isUpdated: boolean }) => {
  return (
    <div className={`w-5 h-5 text-xs flex items-center justify-center border border-white transition-colors duration-500 ${isUpdated ? 'bg-red-300' : 'bg-slate-100'}`}>
      {children}
    </div>
  );
});
...
const SignalBenchmark = ({ setPerfData }: { setPerfData: Function }) => {
  ...
  const gridSignals = useMemo(() => Array.from({ length: CELL_COUNT }, () => createSignal(0)), []);
  ...
  return (
    <div className="benchmark-container">
      <div className="explanation">
        ...
        <p className="text-sm">Only the cell whose signal was changed re-renders. The parent component and the other 2499 cells are not affected. Notice that this container <strong>does not flash</strong> on update.</p>
        <button className="mt-4 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors" onClick={handleUpdate}>Update Random Cell (Signals)</button>
      </div>
      <div className="grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 1fr)` }}>
        {gridSignals.map((signal, i) => (
          <GenericCell key={i} isUpdated={i === lastUpdated}>
            <S>{signal}</S>
          </GenericCell>
        ))}
      </div>
...
const StateBenchmark = ({ setPerfData }: { setPerfData: Function }) => {
  ...
  return (
    <div className={`benchmark-container transition-all duration-300 ${isFlashing ? 'flash-border' : ''}`}>
      <div className="explanation">
        <h3 className="font-bold text-lg mb-2">How It Works (useState)</h3>
        <p className="text-sm">When updating the state, a new array is created, forcing React to re-render the parent component and <strong>all 2500</strong> cells. Notice this container <strong>flashes red</strong> on every click.</p>
        <button className="mt-4 px-4 py-2 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-600 transition-colors" onClick={handleUpdate}>Update Random Cell (useState)</button>
      </div>
      <div className="grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, 1fr)` }}>
...
export function PerformanceBenchmark() {
  ...
  const chartData = [
    { name: 'Average Time (ms)', useState: stats.state.avg, signal: stats.signal.avg },
    { name: 'Last Update (ms)', useState: stats.state.last, signal: stats.signal.last },
  ];
    ...
    <div className="font-sans p-4 md:p-8 w-full bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Benchmark: Signals vs. useState</h1>
          <button onClick={handleReset} className="mt-2 md:mt-0 px-4 py-2 bg-slate-600 text-white text-sm font-semibold rounded-lg hover:bg-slate-700 transition-colors">Reset Statistics</button>
        </div>
        <p className="text-slate-600 mb-8">
          Both tests render a {GRID_SIZE}x{GRID_SIZE} ({CELL_COUNT}) cell grid. Click the button to update the value of <strong>one</strong> random cell and see the difference.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-8">
          <div className="stats-card bg-white p-4 rounded-lg shadow-sm border border-slate-200 text-center">
            ...
            <p className="text-4xl font-bold text-slate-800 mt-1">{statData.times.length}</p>
          </div>
          <div className="stats-card bg-white p-4 rounded-lg shadow-sm border border-slate-200 text-center">
            <h4 className="text-slate-500 font-semibold">Last Time</h4>
            <p className="text-4xl font-bold text-slate-800 mt-1">{statData.last.toFixed(3)} ms</p>
          </div>
          <div className="stats-card bg-white p-4 rounded-lg shadow-sm border border-slate-200 text-center">
            <h4 className="text-slate-500 font-semibold">Average Time</h4>
            <p className="text-4xl font-bold text-slate-800 mt-1">{statData.avg.toFixed(3)} ms</p>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
              ...
              <Tooltip formatter={(value: number) => `${value.toFixed(4)} ms`} cursor={{fill: 'rgba(239, 246, 255, 0.5)'}} />
              <Legend />
              <Bar dataKey="useState" fill="#f97316" name="useState (ms)" />
              <Bar dataKey="signal" fill="#16a34a" name="Signals (ms)" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white p-4 rounded-b-lg shadow-sm border border-t-0 border-slate-200">
          {activeTest === 'signal' && <SignalBenchmark setPerfData={updatePerfData} />}
          {activeTest === 'state' && <StateBenchmark setPerfData={updatePerfData} />}
        </div>
      </div>
    </div>
  );
